...
- **R4 base constraints run with NO profile (`validate/base-invariants.ts`): `dom-3` is DECIDED when
  nothing is contained and UNCHECKED otherwise, never skipped; `dom-6` is NOT evaluated; a contained
  resource's own content is a DECLARED GAP, reached only through `dom-2` to `dom-5`.**
