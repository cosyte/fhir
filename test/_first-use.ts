...
/** Every backtick-quoted literal in a snippet that holds a FHIR JSON resource. */
export function resourceLiterals(code: string): string[] {
  const literals: string[] = [];
  for (const match of code.matchAll(/`([^`]*"resourceType"[^`]*)`/g)) {
 ...
 * A local-path or protocol specifier (`file:../fhir`, `link:`, `git+https:`) installs whatever that
 * path or URL holds rather than resolving a package name, so it is not a registry specifier and is
 * left out.
 */
export function installSpecifiers(markdown: string): string[] {
  ...
  for (const match of markdown.matchAll(command)) {
    const spec = match[2];
    if (spec === undefined || /^[.~/]|^[a-z+]+:/i.test(spec)) continue;
