...
- **`serializeResourceXml` no longer emits a name that authors elements the sender never wrote (`FHIR-UNBOUND-PREFIX-ROUNDTRIP`).**
  ...
  **still written**, because they round-trip through this library unchanged today and refusing them
  ...
  `serializeResourceXml` and pinned by tests, alongside a second one now named there: `Narrative.div`
  is written back as the opaque string the model carries, so an unbalanced one makes the output
  unreadable. **Unreachable for a model read from XML**, whose tag scanner cannot produce such a
  name, so no XML document loses the ability to be written back.
