...
- **FHIR-type tests and `matches()` in the bounded FHIRPath subset, so more of US Core's own
  ...
  data only, never shipped), the engine now evaluates 24 of 29 (6.1.0) and 29 of 34 (9.0.0), up from
  16 and 19; `pnpm uscore:constraints` prints each one evaluated or declined with the construct that
  declined it. A FHIR-type test (`is` / `as` / `ofType`, operator and function forms) is answered
...
- **The documentation sidebar groups its pages into Installation, Quickstart, Core Concepts, Guides
  ...
  counted as either, so the flat list this package shipped left both routes off the site. Current
  limits now sits under Troubleshooting. `scripts/docs-set-check.ts` reads a sidebar written in
  sections: every page id a section reaches, at any depth, must resolve to a page and counts as
  enumerated, and an item it cannot read (a `link`, `html` or `autogenerated` item, or a section
  with no string label or no items array) is a finding rather than skipped. Three seeded controls in
  `test/docs-set.test.ts` show each of those rules going red. No runtime change: nothing under
  `src/` was touched.
