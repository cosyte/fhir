/**
 * The frozen **known-systems registry** (content-free terminology binding).
 *
 * A code lives in a `(system, code)` tuple. This module holds the closed set of code-`system` URIs
 * the library recognizes, the identities from the table, verified against
 ...
 * library bundles **no** LOINC / SNOMED / RxNorm / CPT concept tables (licensing). Whether
 ...
 * URIs; rather than guess one, they are **omitted**, an omitted system reads
 * as "unknown", which is a safe, non-erroring degrade, never a false identity.
 *
 * The `system` constants shared with other layers are re-exported from their owning modules
