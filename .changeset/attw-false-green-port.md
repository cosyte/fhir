...
1. A structural **preflight**. Every relative artifact path `package.json` promises (`main`,
   ...
   file rather than leaving the reader to infer it. The four top-level keys are read in either legal
   spelling, with or without a leading `./`; a string leaf of `exports` must carry the prefix, which
   is Node's own rule, so that arm still requires it. It deliberately does not claim the exit-0
...
than tolerated**. Six were measured here against a `dist/` with both `.d.ts` files deleted, each
exiting 0 with the sentence unreadable: `--quiet`, `--format json`, `-fjson`, `-Pfjson`, a
`./.attw.json` setting `quiet` (`readConfig()` applies it after argv), and `--config-path` pointing
at that same config from elsewhere. The refusal is **by option name, wholesale, not by value**:
`--format table-flipped` prints the sentence and blinds nothing, and is refused anyway, which is a
deliberate trade against adding value-parsing to the guard. Every other argument is forwarded, so
`--profile node16`, `--ignore-rules` and `--no-definitely-typed` still work.

**It takes two arms, and a name match alone is not enough**, which is the one thing the conformance
gate broke on its first pass. commander lets a short option carry its value attached (`-fjson`) and
lets short booleans bundle ahead of it (`-Pfjson`), so both spell `--format json` while carrying no
`=` for a name match to split on, and both were measured handing back a 0 with `"types": false` in a
JSON body. The second arm refuses any single-dash cluster containing `q` or `f`, which is sound
because `-f` is the only short option here that takes a value, so anything after either letter is
that value.
...
the status is not one either. Reverting `scripts/attw.mjs` to the bare invocation reds **15 of its 24
cases**; the 9 that stay green are exactly those asserting upstream `attw` behaviour and the
