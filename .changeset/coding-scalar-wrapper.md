...
An array around `Coding.system`/`Coding.code` in a safety element read as absent, so a refuted allergy read as active, a recorded absence of allergy as an allergy, and a retracted Condition as live.

The element is a `Coding` inside a `CodeableConcept`-valued safety element, and the recorded absence
of allergy is SNOMED `716186003`. With the wrapper unread, that record came back as an **ordinary
live allergy** (`noKnownAllergy` false, `safeToSummarize` true) rather than as the absence it
recorded. The library did not surface `716186003` as the allergen: `codingsOf` yielded the `system`
alone, so the code went missing rather than being reasserted as a substance.
