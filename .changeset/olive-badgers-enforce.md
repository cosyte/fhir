...
generation has no channel to report it on; nothing the base element earned is withdrawn.

One consequence of that is left open rather than guarded, because guarding it was measured to be
worse. Where a profile is contradictory (the base element required, the differential forbidding it
with `0..0`) the composition of the two rules is an unsatisfiable `min 1` beside `max 0`. Every
instance draws a finding from that, but slice resolution reads a descendant's cardinality as an
existence expectation and resolves the contradiction toward "present", so beneath an `exists`
discriminator two slice findings are lost. Clamping the tightened bound against `max` was tried and
reverted: it lowered the enforced bound below the inherited one whenever the differential's own `max`
sat under it, which reaches ordinary profile mistakes rather than only contradictory ones and
validates documents this library previously rejected. The remedy belongs in slice resolution, which
is guessing where its own contract says report the slicing unchecked. It is pinned by a test.
