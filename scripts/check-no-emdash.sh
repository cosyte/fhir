...
# every cosyte surface. Source of truth: the knowledgebase repo,
# 06-brand/voice-and-tone.md ("No em dashes. Ever."), which names commit
# messages explicitly.
#
# This is a regression gate, not a cleanup: when it landed, this repo was
# already clean (175 tracked files, 0 hits), and it exists so it stays that way.
# Ported from knowledgebase's copy, which is the correct variant for a
# text-only repo. Do not swap in website's NUL-partition variant unless this
# repo starts tracking binaries: see the "no -I" note below for what that
# would change.
#
...
# An earlier version of this gate elsewhere discarded that on stderr and `|| true`d
# the pipeline, so it printed OK having scanned nothing.
...
fail_with_hits() {
  ...
  echo "       cosyte never uses em dashes (founder directive; knowledgebase 06-brand/voice-and-tone.md)." >&2
...
# Five things this does, all of them closing a way for the scan to report green
...
#   invalid UTF-8, so an em dash inside one would be skipped silently. Measured at
#   this commit, byte by byte over all 177 tracked blobs: 0 contain a NUL byte and
#   0 contain invalid UTF-8, so this repo has no binaries to skip. (Extensions, for
#   the audit trail: .ts .json .md .xml .yml .yaml .mjs .js .sh .ndjson .txt .toml,
#   the dotfiles, LICENSE, and .github/CODEOWNERS.) Losing -I makes a future binary
#   a loud false positive (grep prints "Binary file X matches", the gate goes red,
#   a human looks) instead of a silent miss. Fail closed, not open.
#
#   KNOWN RESIDUAL, and the thing to re-check before trusting this note: a NUL byte
#   inside a file that is otherwise TEXT produces the same loud red, worded as an
#   incomplete scan, which is confusing rather than actionable. That is not
#   hypothetical here (`feee0dfa` fixed "a stray NUL byte in the PHI-leak diagnostic
#   join"), and this repo's fuzz and PHI-leak corpora make it likelier than in the
#   repo this was ported from. If it recurs, or if a real binary is ever tracked
#   (a fixture image, a compiled artifact), port website's NUL-partition variant
#   rather than adding -I back: -I would trade a loud false positive for a silent
#   miss, which is the wrong direction for a gate.
#
