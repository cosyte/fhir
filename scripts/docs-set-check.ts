...
/**
 * The sidebar as the site reads it: a `docs` array whose items are page ids, `doc` items and
 * categories, flattened to the page ids it reaches.
 *
 * A CATEGORY IS A FIRST-CLASS ENTRY, NOT AN EXOTIC ONE. docs.cosyte.com requires every enabled
 * package to ship Installation and Quickstart as TOP-LEVEL CATEGORIES that reach a page, and it
 * does not count a bare `installation` or `quickstart` page id as either. So the sidebar this
 * package ships has to be written in categories, and a checker that refused them would forbid the
 * only shape the site accepts. Every page id a category reaches, at any depth, is enumerated
 * exactly as a top-level id is, so both rules below still bind inside a category: an entry that
 * resolves to no page is a finding, and a page no entry reaches is a finding.
 *
 * Any other item shape (`link`, `html`, `autogenerated`, a category without a string `label` or
 * an `items` array) is REFUSED rather than skipped, so an entry this check cannot read is a
...
function readSidebar(dir: string): { entries: string[]; error: string | undefined } {
  ...
  const error = collectSidebarIds(docs, entries);
  return { entries, error };
}

/**
 * Append the page ids a list of sidebar items reaches to `into`, depth first and in order.
 * Returns why an item cannot be read, or `undefined` when every item was read.
 */
function collectSidebarIds(items: readonly unknown[], into: string[]): string | undefined {
  for (const item of items) {
    ...
    if (typeof item !== "object" || item === null || Array.isArray(item)) {
      return "every sidebar item must be a page id string, a `doc` item or a `category`";
    }
    ...
    if (type === "doc" && typeof id === "string") {
      into.push(id);
      continue;
    }
    if (type === "category" && typeof label === "string" && Array.isArray(children)) {
      const nested = collectSidebarIds(children, into);
      if (nested !== undefined) return nested;
      continue;
    }
    return (
      "every sidebar item must be a page id string, a `doc` item with a string `id`, or a " +
      "`category` with a string `label` and an `items` array"
    );
  }
  return undefined;
}
