...
#
# The `dist` exclusion is spelled BOTH ways on purpose. `files` names the four build
# artifacts one by one instead of the `dist` directory, so that the two sourcemaps stay out
# of the tarball. Those four paths are the SAME untracked build output the `dist` entry
# always stood for, excluded for the identical reason and gated at the same place: their
# source, by the third pass. Naming them changes what npm packs, and nothing about what this
# gate reads. Should `files` go back to naming the directory, the `dist` entry is still here
# and still works. Any other path under `dist/` is still unknown and still refused.
